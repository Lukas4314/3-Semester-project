...
Communication between the Raspberry Pi and the PC is handled using MQTT (Message Queuing Telemetry Transport) protocol. MQTT uses a publisher-subscriber model, where a publisher sends messages to a topic hosted on a broker, which then is sent to all subscribers (all components that need data from that topic) \cite{OASIS_MQTT5}. %This ensures a layered architecture where different components only know about the topics they need data from, instead of all other components, allowing for modularity and easy scalability. 
...
In this project the python library paho-mqtt handles the low-level details of the MQTT protocol and parsing. % Indsæt kilde for paho-mqtt
