...
While the motors are driving, the OpenCR board sends positional data from the encoders on the board back to the ROS2 environment. This makes it possible to drive the exact distance by continuously checking for position until the goal is reached. By calculating the position in the desired path, it also becomes possible to ramp up slowly at the start and ramp down at the end, which eliminates overshooting due to momentum.
