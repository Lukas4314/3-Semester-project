...
When the audio data has been captured from the microphone, sent to the Raspberry Pi and published on a MQTT topic, the PC needs to transcribe the audio data into text for it to be useful. This is done on the processing layer using OpenAI's Whisper model.\cite{whisper_paper} 
...
A seperate thread is allocated exclusivily for general transcription, as it is processing a large data stream at a time, and therefore could block other processes. With this approach, the audio input for the transcription model is handled using a queue, that stores incoming audio data until the transcription thread is ready to process it.

As multiple threads need to interact with the Whisper model simultaneously, a mutex lock is used to ensure that requests to the model arrive one at a time, despite the asynchronous behaviour.

For general transcription, the audio data is analysed every 2.5 seconds, with an analysis window of 5 seconds. This means that every 2.5 seconds, the last 5 seconds of audio is sent to the Whisper model for transcription, giving an overlap of 2.5 seconds that will be analysed twice. This overlap is necessary as words, phrases or commands could be cut off at the boundaries of the audio stream, and would therefore not be transcribed correctly. Since this overlap results in duplicate transcriptions, a check is implemented to ensure only new information is provided for string analysis. This is done by comparing every new transcription with the previous one. Any new part is added to an output queue that can then be concatenated into a string for command identification seperately from the transcription thread.

The Whisper model has different options depending on the use case. To determine which options to use on the system, a script was made to run through 767 different combinations of options on 10 different sound clips. This resulted in a temperature of 0.3, which defines how creative it is allowed to be, where 0 is very deterministic and 1 is very creative, meaning any noise would be transcribed into words. All other options were set to default as it proved to be the most optimal.
